...
class SpeechManager:
    def __init__(self, processor_model="./checkpoint-4", vocoder_model="microsoft/speecht5_hifigan", seed=555):
        self.processor = SpeechT5Processor.from_pretrained(processor_model)  # Initialize processor from local checkpoint
        self.model = SpeechT5ForSpeechToSpeech.from_pretrained(processor_model)  # Initialize model from local checkpoint
        self.vocoder = SpeechT5HifiGan.from_pretrained(vocoder_model)  # Initialize vocoder from local checkpoint
